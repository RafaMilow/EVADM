...
class Evento extends BancoPDO
{
    ...
    public function buscaTodos()
    {
        ...
        $_sql = sprintf(
            "	SELECT id_evento, titulo, cidade, local, data_inicio, data_fim
							  FROM evnts_evento 
    ...
    public function buscaEventoMaisRecente()
    {
        ...
        $_sql = sprintf(
            "	SELECT 	id_evento, titulo, chamada_1, chamada_2, CTA, CTA_DETALHE, 
    ...
    public function retornaJSON()
    {
        ...
        $_sql = sprintf(
            "	SELECT id_evento, titulo, data_inicio, data_fim, lat, lng, endereco, cidade, local, zoom
							FROM evnts_evento
							WHERE id_evento = %d 
							AND situacao like 'A';", $this->_evento
        );
    ...
    public function buscaTipoAcomodacoes()
    {
        ...
        $_sql = sprintf(
            "	SELECT COUNT(DISTINCT(h.tipo_acomodacao)) AS tipos 
